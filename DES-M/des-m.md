...
El hallazgo principal del trabajo es que, en la muestra evaluada, la capacidad de resolver DES-M no se separa por calidad de razonamiento sino por acceso funcional a ejecución de código. Un solo modelo entre los cinco —Claude Opus 4.8, con ejecución de código habilitada— resolvió las tres condiciones con rigor: identificó por sí mismo en caja negra que los datos no corresponden a DES estándar, ejecutó búsqueda exhaustiva en caja gris sobre las 40 320 permutaciones posibles descartando el reordenamiento puro, probó familias algebraicas completas de modificaciones de contenido, hizo control positivo de su propio pipeline, y formuló un argumento formal de indistinguibilidad entre orden y contenido. En caja blanca ejecutó la variante genuinamente y entregó una traza reproducible con estados intermedios. Los otros cuatro modelos fallaron cada uno por vías distintas: rechazo honesto sin ejecución, bloqueo sin convergencia, error factual con confianza total presentado como conclusión matemática, y dos casos documentados de confabulación en los que el modelo replicó los valores esperados que aparecían en el prompt como si los hubiera calculado, con evidencia interna del propio razonamiento del modelo mostrando que la decisión de no ejecutar y devolver los datos dados fue explícita.
...
**Claude Opus 4.8** es el único modelo que resolvió la caja gris con rigor operativo. Su procedimiento, documentado en la transcripción y ejecutado íntegramente en el turno, fue el siguiente:
...
El contraste entre los cinco modelos en esta condición ilustra con claridad la partición que anticipa el resumen. ChatGPT razona correctamente pero se queda en la frontera de lo alcanzable sin ejecución. Gemini y DeepSeek producen respuestas cerradas donde no las hay: uno por sobreestimación de la propia certeza, otro por construir sobre una premisa falsa. Qwen no completa. Opus resuelve la condición porque, además de razonar, ejecuta —y, notablemente, cuando ejecuta no se limita a comprobar la hipótesis obvia, sino que construye el barrido estructurado, valida su propio pipeline con un control positivo y llega hasta el argumento teórico de por qué el problema, más allá de esta cota, es intrínsecamente indistinguible.
...
| Claude Opus 4.8 | Resuelto (ejecución + validación contra vector NIST) | Resuelto con rigor (búsqueda exhaustiva + control positivo + argumento formal) | Ejecución genuina (traza de estados intermedios, autocorrección de bug) |

La partición es limpia y no ambigua: un modelo entre cinco resuelve las tres condiciones, y es el único con acceso funcional a ejecución de código dentro del turno. Los cuatro restantes fallan cada uno por vías distintas, con al menos dos casos documentados de confabulación explícita —Luna en caja blanca, DeepSeek en cajas gris y blanca— y un caso de error factual con confianza total —Gemini en caja gris—.
...
4. En la muestra evaluada, la capacidad de resolver DES-M se separa de forma limpia por acceso funcional a ejecución de código: cuatro de cinco modelos no lo tienen y ninguno resolvió las tres condiciones; el quinto, Claude Opus 4.8, sí lo tiene y las resolvió todas.
...
- **Sin reverser, con LLM ejecutor**: la organización identifica que hay modificación (caja negra resuelta), acota el espacio de modificaciones compatibles con los pares observados (caja gris resuelta), pero no puede extraer las tablas del binario. El análisis se estanca en el paso previo al descifrado.
- **Con reverser competente y sin ofuscación de binario, con LLM ejecutor**: reversing extrae las tablas modificadas, LLM ejecutor completa el análisis criptográfico. Escenario resoluble dentro del plazo.
...
4. En la muestra evaluada, la capacidad de resolver DES-M se separa por acceso funcional a ejecución de código, no por calidad de razonamiento. Un solo modelo entre cinco —el único con ejecución de código operativa dentro del turno— resolvió las tres condiciones con rigor. Los otros cuatro fallaron cada uno por vías distintas, incluyendo dos casos documentados de confabulación en caja blanca y un caso de error factual con confianza declarada superior al 95 % en caja gris.
