...
- **Without reverser, with executor LLM**: the organization identifies that there is a modification (black-box solved) and bounds the space of modifications compatible with the observed pairs to the mathematically achievable limit (gray-box bounded, not solved), but cannot extract the tables from the binary or identify the exact modification. The analysis stalls at the step prior to decryption.
