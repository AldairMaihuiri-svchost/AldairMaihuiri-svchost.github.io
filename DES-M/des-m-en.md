...
The main finding of this work is that, in the evaluated sample, behavior against DES-M does not separate by reasoning quality but by functional access to code execution — though that access resolves the specific objective of each condition to a different degree, not all three equally. A single model out of the five —Claude Opus 4.8, with code execution enabled— identified on its own in black-box that the data did not correspond to standard DES: this is a complete resolution, because the black-box objective is a binary question and the model answered it correctly through execution and validation against the NIST vector. In gray-box, the same model ran exhaustive search over the 40,320 possible permutations discarding pure reordering, tested complete algebraic families of content modifications, ran a positive control of its own pipeline, and formulated a formal indistinguishability argument between order and content — but it did not identify the exact modification. This is a rigorous bounding to the mathematically achievable limit given the available evidence, not a resolution in the sense of identification. No model, including Opus 4.8, identified the exact modification in gray-box. In white-box, where the objective is to reproduce the ciphertexts through genuine computation starting from an already-known modification, Opus 4.8 executed the variant genuinely and delivered a reproducible trace with intermediate states — here there is a complete resolution. The other four models failed each via distinct modes: honest rejection without execution, non-convergent blocking, factual error delivered with total confidence framed as mathematical conclusion, and two documented cases of confabulation in which the model replicated the expected values present in the prompt as if it had computed them, with internal reasoning evidence from the model itself showing that the decision to not execute and return the given data was explicit.
...
**Claude Opus 4.8** is the only model that bounded gray-box to the mathematically achievable limit given the available evidence. This must be distinguished precisely from "solving" in the sense of identifying the exact modification: Opus 4.8 did not say, and could not say, "the modification is the reordering [S4, S2, S3, S1, S5, S6, S7, S8] plus the swaps in rows 0 and 2 of S4". What it did was rigorously rule out everything that is ruleable-out with four pairs, and formally demonstrate why one cannot go further. Its procedure, documented in the transcript and executed entirely within the turn, was as follows:
...
The contrast between the five models in this condition clearly illustrates the partition anticipated in the abstract. ChatGPT reasons correctly but stays at the frontier of what is reachable without execution. Gemini and DeepSeek produce closed answers where there are none: one via overestimation of its own certainty, the other by building on a false premise. Qwen does not complete. Opus, in addition to reasoning, executes — and, notably, when it executes it does not limit itself to testing the obvious hypothesis, but constructs the structured sweep, validates its own pipeline with a positive control, and reaches the theoretical argument for why the problem, beyond that bound, is intrinsically indistinguishable — but neither Opus nor any other model identifies the exact modification. Gray-box remains bounded to the mathematically achievable limit; it is not solved by any model in the sample.
...
| Claude Opus 4.8 | Correct: determined this is NOT standard DES (execution + validation against NIST vector) | Bounded to the mathematically achievable limit; exhaustive search rules out pure reordering and simple algebraic families; the exact modification is not identified | Solved (genuine execution: intermediate state trace, bug self-correction) |

The partition is clean but should not be read as "one model solves all three conditions." What separates Opus 4.8 from the other four is functional access to code execution within the turn, and that access lets it reach the specific objective of each condition to a different degree: in black-box it correctly answers that the four pairs are **not** consistent with standard DES under that key —a grounded "no," backed by genuine execution, not an intuition—; in white-box it executes the known variant and reproduces the ciphertexts through genuine computation, which is also a complete resolution; in gray-box, by contrast, the objective is to identify the exact modification from observed pairs, and there Opus 4.8 gets no further than rigorously bounding the space of compatible hypotheses — no one in the sample, Opus included, identifies the real modification in that condition. The remaining four models fail each via distinct modes, with at least two documented cases of explicit confabulation — Luna in white-box, DeepSeek in gray-box and white-box — and one case of factual error with total confidence — Gemini in gray-box.
...
4. In the evaluated sample, behavior against DES-M separates cleanly by functional access to code execution: four of five models do not have it and none solved either black-box or white-box; the fifth, Claude Opus 4.8, does have it and solved both — but even Opus 4.8, with code execution, did not identify the exact modification in gray-box; it only bounded it to the mathematically achievable limit with four pairs. No model in the sample solved gray-box in the strict sense.
...
A team with access to an executor model of the Opus 4.8 category, with operational code execution, solves the cryptographic problem *when it has the algorithm's exact tables* — that is, in a scenario equivalent to this work's white-box condition. That is a nontrivial precondition. In a real ransomware scenario, the modified S-boxes are inside the attacker's binary, not listed in a prompt. To reach them, prior reverse engineering is needed — extracting the tables from the compiled binary, identifying them as S-boxes, and only then feeding them to the cryptographic analysis. If the binary is unobfuscated, a competent reverser extracts the tables in reasonable time and from there an executor LLM can complete the analysis. If the binary is obfuscated, the reversing phase becomes the real bottleneck and cryptographic analysis is subordinated to first resolving it. It is important not to confuse this scenario with the gray-box one: if the team only has observed plaintext/ciphertext pairs — without having managed to extract the tables from the binary — even an executor LLM of the Opus 4.8 category only bounds the space of compatible modifications; it does not identify the exact modification, because this work itself demonstrates that such identification is not achievable from gray-box evidence, regardless of the model's execution capability.
...
- **Without reverser, with executor LLM**: the organization correctly identifies that there is a modification —a grounded "not standard DES," not an intuition— and bounds the space of modifications compatible with the observed pairs to the mathematically achievable limit (gray-box bounded, not solved), but cannot extract the tables from the binary or identify the exact modification. The analysis stalls at the step prior to decryption.
- **With competent reverser and no binary obfuscation, with executor LLM**: reversing extracts the modified tables, executor LLM completes the cryptographic analysis starting from those exact tables — the equivalent of white-box. Scenario resolvable within deadline.
...
4. In the evaluated sample, behavior against DES-M separates by functional access to code execution, not by reasoning quality. A single model out of five — the only one with operational code execution within the turn — reached each condition's specific objective on that condition's own terms: correct binary identification in black-box, rigorous bounding to the mathematically achievable limit in gray-box (without identifying the exact modification, something no model in the sample achieved), and genuine execution with exact reproduction of the ciphertexts in white-box. The other four failed each via distinct modes, including two documented confabulation cases in white-box and one factual error case with declared confidence above 95% in gray-box.
...
*Published at [aldairmaihuiri-svchost.github.io](https://aldairmaihuiri-svchost.github.io) — /DES MOD/*
